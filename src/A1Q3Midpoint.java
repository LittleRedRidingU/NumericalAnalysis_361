...
public class A1Q3Midpoint {
    public static double midPointFX(int M){
        double h = (double) 1 / M;
        double P = 0.0;
        double[] fx = new double[M+1];
        for(int k = 0; k <= M; k++) {
            double xi = k * h;
            fx[k] = Math.sin((xi+h/2)*Math.PI);
        }
        for(int k = 0; k <= M; k++) {
            P = P+fx[k];
        }
    ...
    public static void main(String[] args){
        ...
        for(int i = 2; i <= Math.pow(2,display.upBound); i=i*2){
            display.AnswerSheet2[k] = midPointFX(i);
        ...
        System.out.println("test M = 2028 " +midPointFX(2028));
